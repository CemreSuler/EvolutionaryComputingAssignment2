...
if mode == "test":
    ...
    for i, group in enumerate(list(set(df["group"]))):
        ...
        dead_enemies = (
            np.count_nonzero(df["enemy_life"].loc[this_group] == 0) / repetitions
        )
        gain = (
            sum(df["player_life"].loc[this_group] - df["enemy_life"].loc[this_group])
            / repetitions
        )
    ...
    winners_slain = df_final.sort_values(
        by=["enemies_slain", "player_life", "time"], ascending=False
    ).reset_index()
    ...
    for winner in [
        ...
        for group in winner:
            ...
            if group == "whole_class":
                df_plot = (
                    df.drop(["group", "repetition"], axis=1)
                    .apply(pd.to_numeric)
                    .groupby("enemy")
                    .mean()
                    .transpose()
